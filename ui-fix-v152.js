...
function hideLegacyVersions(){
  ...
  bottom.querySelectorAll('*').forEach(el=>{
    if(el.id==='appVersionV152')return;
  ...
  let current=document.getElementById('appVersionV152');if(!current){current=document.createElement('div');current.id='appVersionV152';bottom.appendChild(current);}
...
function ensureVisibleDebtTitle(){const board=document.querySelector('#debtListSection .boardHeader');if(!board)return;const originals=[...board.querySelectorAll('h2')].filter(x=>x.id!=='v152DebtTitle');originals.forEach(x=>x.style.setProperty('display','none','important'));let title=document.getElementById('v152DebtTitle');if(!title){title=document.createElement('h2');title.id='v152DebtTitle';board.prepend(title);}title.textContent='Schulden ('+renderedDebtCount()+')';}
function updateModalTitle(){document.querySelectorAll('.v132Modal h2').forEach(el=>{if(/Insolvenz-Status/i.test(el.textContent||''))el.textContent='Insolvenz-Status '+VERSION;});document.querySelectorAll('.cgiModal .cgiHead h3').forEach(el=>{if(/ChatGPT Import/i.test(el.textContent||''))el.textContent='🤖 ChatGPT Import '+VERSION;});}
...
const oldRender=window.render;if(typeof oldRender==='function'&&!oldRender.__v152UiFix){const wrapped=function(){const r=oldRender.apply(this,arguments);refreshLater();try{window.v152DashboardRefresh?.();}catch(e){}return r;};wrapped.__v152UiFix=true;window.render=wrapped;try{render=wrapped;}catch(e){}}
const oldOpen=window.v132InsolvenzOpen;if(typeof oldOpen==='function'&&!oldOpen.__v152UiFix){const wrappedOpen=function(){const r=oldOpen.apply(this,arguments);refreshLater();return r;};wrappedOpen.__v152UiFix=true;window.v132InsolvenzOpen=wrappedOpen;}

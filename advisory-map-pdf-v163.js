...
function deadlineFor(d,m){
  const insolvency=validDate(m?.dueDate);if(insolvency)return {date:insolvency,source:'Frist'};
  const legacy=validDate(d?.datum);if(legacy)return {date:legacy,source:'Alt-Datum'};
  return {date:'',source:''};
}
function deadlineLabel(d,m){const x=deadlineFor(d,m);return x.date?(x.source+': '+fmtDate(x.date)):'–';}
...
function exportPdf(){
  ...
  try{
    ...
    const body=list.map((d,i)=>{
      const m=all[keyFor(d,i)]||{},c=contact(d),miss=missingItems(d,i,m);
      return [d?.name||'',money(d?.betrag),caseNo(d)||'',c.address||'',deadlineLabel(d,m),String(documents(d).length),m.locked?'geprüft':'offen',miss.join(', ')||'–'];
    });
    if(typeof doc.autoTable==='function')doc.autoTable({startY:25,head:[['Gläubiger','Betrag','Aktenzeichen','Anschrift','Frist / Alt-Datum','Dok.','Status','Fehlt / offen']],body,styles:{fontSize:7,cellPadding:1.5},headStyles:{fontSize:7},columnStyles:{0:{cellWidth:35},1:{cellWidth:23},2:{cellWidth:30},3:{cellWidth:52},4:{cellWidth:30},5:{cellWidth:12},6:{cellWidth:18},7:{cellWidth:67}}});
    ...
    let y=(doc.lastAutoTable?.finalY||25)+8;if(y>190){doc.addPage();y=14;}doc.setFontSize(7);doc.text('Hinweis: „Frist“ = ausdrücklich im Insolvenz-Arbeitsbereich erfasste Frist. „Alt-Datum“ = bestehendes altes Datumsfeld des Schuldeneintrags und nicht automatisch eine rechtliche Frist. Organisatorische Arbeitsunterlage; keine rechtliche Prüfung.',14,y,{maxWidth:268});
...
window.v163ExportAdvisoryMapPdf=exportPdf;
})();

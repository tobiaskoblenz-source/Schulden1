...
const DATA_DIR=process.env.DATA_DIR||path.join(__dirname,'data');const FILE=path.join(DATA_DIR,'schulden-users-v170.json');const KEYS=['overview','creditors','details','notes','documents','paperless','compare','download'];
const ALL=Object.fromEntries(KEYS.map(k=>[k,true]));const DEF={...ALL};
...
function write(users){ensure();const t=FILE+'.tmp';fs.writeFileSync(t,JSON.stringify({version:170,updatedAt:new Date().toISOString(),users},null,2));fs.renameSync(t,FILE)}
...
module.exports={DATA_DIR,FILE,KEYS,ALL,DEF,read,write,perms,pub,username,name,role,hash,verify};

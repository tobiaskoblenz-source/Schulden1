...
function ocrStartHay(doc){
  // Nur der Dokumentanfang darf für EINDEUTIGE einzelne Gläubigernamen zählen.
  // So kann z.B. Refood im Briefkopf erkannt werden, während mehrteilige Namen wie
  // "AOK Sachsen-Anhalt" nicht wegen einer beiläufigen OCR-Erwähnung vorausgewählt werden.
  return norm(String(doc?.content||'').slice(0,1200));
}
...
window.addEventListener('change',e=>{
  const sel=e.target?.closest?.('[data-v153-pl-select]');if(sel)sel.dataset.v157Manual='1';
},true);
...
window.v157PaperlessInboxFilter=apply;
})();

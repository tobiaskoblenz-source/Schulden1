...
function hideLegacyVersions(){
  ...
  bottom.querySelectorAll('*').forEach(el=>{
    if(el.id==='appVersionV153')return;
  ...
  let current=document.getElementById('appVersionV153');if(!current){current=document.createElement('div');current.id='appVersionV153';bottom.appendChild(current);}
...
function ensureVisibleDebtTitle(){const board=document.querySelector('#debtListSection .boardHeader');if(!board)return;const originals=[...board.querySelectorAll('h2')].filter(x=>x.id!=='v153DebtTitle');originals.forEach(x=>x.style.setProperty('display','none','important'));let title=document.getElementById('v153DebtTitle');if(!title){title=document.createElement('h2');title.id='v153DebtTitle';board.prepend(title);}title.textContent='Schulden ('+renderedDebtCount()+')';}
...
const oldRender=window.render;if(typeof oldRender==='function'&&!oldRender.__v153UiFix){const wrapped=function(){const r=oldRender.apply(this,arguments);refreshLater();return r;};wrapped.__v153UiFix=true;window.render=wrapped;try{render=wrapped;}catch(e){}}
const oldOpen=window.v132InsolvenzOpen;if(typeof oldOpen==='function'&&!oldOpen.__v153UiFix){const wrappedOpen=function(){const r=oldOpen.apply(this,arguments);refreshLater();return r;};wrappedOpen.__v153UiFix=true;window.v132InsolvenzOpen=wrappedOpen;}

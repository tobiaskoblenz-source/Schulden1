...
function legacySuggestion(sel,small){
  const stored=sel.dataset.v159LegacyIndex;
  if(stored!==undefined){
    if(stored===''||stored==='none')return null;
    const i=Number(stored),score=Number(sel.dataset.v159LegacyScore||0);
    if(Number.isInteger(i)&&i>=0&&score>=60&&debtsArr()[i])return {i,score,reason:'starker vorhandener Treffer'};
    return null;
  }
  const raw=String(small?.textContent||'');
  const m=raw.match(/Vorschlag:\s*(.*?)\s*\((\d+)\)\s*$/i);
  const rawValue=String(sel.value??'').trim();
  const i=rawValue===''?NaN:Number(rawValue);
  const score=m?Number(m[2]||0):0;
  const d=Number.isInteger(i)&&i>=0?debtsArr()[i]:null;
  const name=norm(d?.name),tokens=name.split(' ').filter(Boolean);
  let trusted=false;
  if(m&&d&&score>=60){
  ...
  if(trusted){
    sel.dataset.v159LegacyIndex=String(i);
    sel.dataset.v159LegacyScore=String(score);
    return {i,score,reason:'starker vorhandener Treffer'};
  }
  sel.dataset.v159LegacyIndex='none';
  sel.dataset.v159LegacyScore='0';
  return null;
}
...
function apply(){
  ...
  rows.forEach(row=>{
    ...
    const legacy=legacySuggestion(sel,small);
...
window.addEventListener('change',e=>{const sel=e.target?.closest?.('[data-v153-pl-select]');if(sel)sel.dataset.v159Manual='1';},true);
...
window.v159PaperlessInboxFilter=apply;
})();

...
function hideLegacyVersions(){
  ...
  bottom.querySelectorAll('*').forEach(el=>{
    if(el.id==='appVersionV147')return;
  ...
  let current=document.getElementById('appVersionV147');
  if(!current){current=document.createElement('div');current.id='appVersionV147';bottom.appendChild(current);}
...
function ensureVisibleDebtTitle(){
  ...
  const originals=[...board.querySelectorAll('h2')].filter(x=>x.id!=='v147DebtTitle');
  originals.forEach(x=>x.style.setProperty('display','none','important'));
  let title=document.getElementById('v147DebtTitle');
  if(!title){title=document.createElement('h2');title.id='v147DebtTitle';board.prepend(title);}
...
if(typeof oldRender==='function'&&!oldRender.__v147UiFix){
  const wrapped=function(){const r=oldRender.apply(this,arguments);refreshLater();return r;};
  wrapped.__v147UiFix=true;window.render=wrapped;try{render=wrapped;}catch(e){}
}
const oldOpen=window.v132InsolvenzOpen;
if(typeof oldOpen==='function'&&!oldOpen.__v147UiFix){
  const wrappedOpen=function(){const r=oldOpen.apply(this,arguments);refreshLater();return r;};
  wrappedOpen.__v147UiFix=true;window.v132InsolvenzOpen=wrappedOpen;
}

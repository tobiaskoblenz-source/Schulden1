...
function hideLegacyVersions(){
  ...
  bottom.querySelectorAll('*').forEach(el=>{
    if(el.id==='appVersionV156')return;
  ...
  let current=document.getElementById('appVersionV156');if(!current){current=document.createElement('div');current.id='appVersionV156';bottom.appendChild(current);}
...
function ensureVisibleDebtTitle(){const board=document.querySelector('#debtListSection .boardHeader');if(!board)return;const originals=[...board.querySelectorAll('h2')].filter(x=>x.id!=='v156DebtTitle');originals.forEach(x=>x.style.setProperty('display','none','important'));let title=document.getElementById('v156DebtTitle');if(!title){title=document.createElement('h2');title.id='v156DebtTitle';board.prepend(title);}title.textContent='Schulden ('+renderedDebtCount()+')';}
...
const oldRender=window.render;if(typeof oldRender==='function'&&!oldRender.__v156UiFix){const wrapped=function(){const r=oldRender.apply(this,arguments);refreshLater();return r;};wrapped.__v156UiFix=true;window.render=wrapped;try{render=wrapped;}catch(e){}}
const oldOpen=window.v132InsolvenzOpen;if(typeof oldOpen==='function'&&!oldOpen.__v156UiFix){const wrappedOpen=function(){const r=oldOpen.apply(this,arguments);refreshLater();return r;};wrappedOpen.__v156UiFix=true;window.v132InsolvenzOpen=wrappedOpen;}

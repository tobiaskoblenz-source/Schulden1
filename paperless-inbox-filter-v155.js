...
function headerHay(doc){
  return norm([
    doc?.title,doc?.original_filename,doc?.original_file_name,doc?.archive_filename,doc?.archived_file_name,
    objText(doc?.correspondent)
  ].filter(Boolean).join(' '));
}
function identifierHay(doc){
  return compact([
    doc?.title,doc?.original_filename,doc?.original_file_name,doc?.archive_filename,doc?.archived_file_name,
    objText(doc?.correspondent),doc?.content
  ].filter(Boolean).join(' '));
}
function strictSuggestion(doc){
  const head=headerHay(doc),ids=identifierHay(doc),candidates=[];
  debtsArr().forEach((d,i)=>{
    const az=compact(caseNo(d)),cust=compact(customerNo(d)),name=norm(d?.name);
    let score=0,reason='';
    ...
    // Namen werden absichtlich NICHT im OCR-Volltext gesucht. Nur Titel, Dateiname oder Paperless-Absender zählen.
...
function cleanMeta(s){
  return String(s||'')
    .replace(/\s*·\s*Vorschlag:.*$/i,'')
    .replace(/\s*·\s*Sicherer Vorschlag:.*$/i,'')
    .replace(/\s*·\s*Kein sicherer Vorschlag.*$/i,'')
    .trim();
}
function apply(){
  ...
  rows.forEach(row=>{
    ...
    const suggestion=strictSuggestion(doc),base=cleanMeta(small.textContent);
    if(suggestion){
      if(sel.dataset.v155Manual!=='1')sel.value=String(suggestion.i);
      const name=debtsArr()[suggestion.i]?.name||'Gläubiger';
      small.textContent=base+' · Sicherer Vorschlag: '+name+' ('+suggestion.reason+')';
      sel.dataset.v155Safe='1';
    }else{
      if(sel.dataset.v155Manual!=='1')sel.value='';
      small.textContent=base+' · Kein sicherer Vorschlag';
      sel.dataset.v155Safe='0';
    }
  });
  return true;
}
...
async function waitThenFilter(id,attempt=0){
  ...
  if(!rows.length){if(attempt<10)setTimeout(()=>waitThenFilter(id,attempt+1),350);return;}
  try{await fetchDocs();}catch(e){}
  if(id!==runId)return;
  apply();
  setTimeout(()=>{if(id===runId)apply();},500);
}
function start(){const id=++runId;setTimeout(()=>waitThenFilter(id,0),250);}

document.addEventListener('change',e=>{
  const sel=e.target?.closest?.('[data-v153-pl-select]');if(sel)sel.dataset.v155Manual='1';
},true);
document.addEventListener('click',e=>{
  if(e.target?.closest?.('[data-v153-pl-load]'))start();
  if(e.target?.closest?.('[data-v153-tab="paperless"]'))setTimeout(()=>{apply();},300);
},true);

window.v155PaperlessInboxFilter=apply;
})();

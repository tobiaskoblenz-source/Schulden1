...
function hideLegacyVersions(){
  ...
  bottom.querySelectorAll('*').forEach(el=>{
    if(el.id==='appVersionV146')return;
  ...
  let current=document.getElementById('appVersionV146');
  if(!current){current=document.createElement('div');current.id='appVersionV146';bottom.appendChild(current);}
...
function ensureVisibleDebtTitle(){
  ...
  const originals=[...board.querySelectorAll('h2')].filter(x=>x.id!=='v146DebtTitle');
  originals.forEach(x=>x.style.setProperty('display','none','important'));
  let title=document.getElementById('v146DebtTitle');
  if(!title){title=document.createElement('h2');title.id='v146DebtTitle';board.prepend(title);}
...
if(typeof oldRender==='function'&&!oldRender.__v146UiFix){
  const wrapped=function(){const r=oldRender.apply(this,arguments);refreshLater();return r;};
  wrapped.__v146UiFix=true;window.render=wrapped;try{render=wrapped;}catch(e){}
}
const oldOpen=window.v132InsolvenzOpen;
if(typeof oldOpen==='function'&&!oldOpen.__v146UiFix){
  const wrappedOpen=function(){const r=oldOpen.apply(this,arguments);refreshLater();return r;};
  wrappedOpen.__v146UiFix=true;window.v132InsolvenzOpen=wrappedOpen;
}

...
function hideLegacyVersions(){
  ...
  bottom.querySelectorAll('*').forEach(el=>{
    if(el.id==='appVersionV155')return;
  ...
  let current=document.getElementById('appVersionV155');if(!current){current=document.createElement('div');current.id='appVersionV155';bottom.appendChild(current);}
...
function ensureVisibleDebtTitle(){const board=document.querySelector('#debtListSection .boardHeader');if(!board)return;const originals=[...board.querySelectorAll('h2')].filter(x=>x.id!=='v155DebtTitle');originals.forEach(x=>x.style.setProperty('display','none','important'));let title=document.getElementById('v155DebtTitle');if(!title){title=document.createElement('h2');title.id='v155DebtTitle';board.prepend(title);}title.textContent='Schulden ('+renderedDebtCount()+')';}
...
const oldRender=window.render;if(typeof oldRender==='function'&&!oldRender.__v155UiFix){const wrapped=function(){const r=oldRender.apply(this,arguments);refreshLater();return r;};wrapped.__v155UiFix=true;window.render=wrapped;try{render=wrapped;}catch(e){}}
const oldOpen=window.v132InsolvenzOpen;if(typeof oldOpen==='function'&&!oldOpen.__v155UiFix){const wrappedOpen=function(){const r=oldOpen.apply(this,arguments);refreshLater();return r;};wrappedOpen.__v155UiFix=true;window.v132InsolvenzOpen=wrappedOpen;}
